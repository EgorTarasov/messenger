...
export const MessageBubble = ({ message, isOwnMessage, isGroup }: MessageBubbleProps) => {
    ...
    useEffect(() => {
        if (message.expand && message.expand.author) {
            getUserAvatar(message.expand.author).then(url => {
                setAvatarUrl(url);
            }).catch(error => {
        ...
        return <>
            <div className={`flex justify-end  mb-4`}>
                <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
                    ...
                    <DropdownMenuContent>
                        <DropdownMenuItem onClick={handleCopyText}>Копировать</DropdownMenuItem>
                        <DropdownMenuItem onClick={handleDeleteMessage}>Удалить</DropdownMenuItem>
                    </DropdownMenuContent>
                </DropdownMenu >
            </div>
        </>
